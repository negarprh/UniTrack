...
struct AuthView: View {
  ...
  @State private var selectedRole = 0        // 0 = student, 1 = teacher
  ...
  var body: some View {
    ZStack {
      ...
      VStack(spacing: 22) {
        ...
        .overlay(RoundedRectangle(cornerRadius: 18).stroke(.white.opacity(0.35), lineWidth: 0.5))
        .padding(.horizontal, 22)

        Spacer(minLength: 0)
      }
      .padding(.top, 40)
      // Alert feedback for reset and other messages
  ...
  private func submit() {
    ...
    if mode == 0 {
      ...
      let role = (selectedRole == 0) ? "student" : "teacher"
      vm.signUp(email: e, password: p, role: role)
    }
  }
}
...
private struct FieldStyle: ViewModifier {
  func body(content: Content) -> some View {
    content
      ...
      .overlay(RoundedRectangle(cornerRadius: 14).stroke(.secondary.opacity(0.25), lineWidth: 1))
  }
}
